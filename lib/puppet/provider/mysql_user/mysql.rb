...
Puppet::Type.type(:mysql_user).provide(:mysql,
	...
	def self.instances
		...
		cmd = "#{command(:mysql)} --defaults-file=/root/.my.cnf mysql -NBe 'select concat(user, \"@\", host), password from user'"
	...
	def query
		...
		cmd = "#{command(:mysql)} --defaults-file=/root/.my.cnf -NBe 'select concat(user, \"@\", host), password from user where concat(user, \"@\", host) = \"%s\"'" % @resource[:name]
		execpipe(cmd) do |process|
			process.each do |line|
				unless result.empty?
					raise Puppet::Error,
	...
	def create
		mysql "--defaults-file=/root/.my.cnf", "mysql", "-e", "create user '%s' identified by PASSWORD '%s'" % [ @resource[:name].sub("@", "'@'"), @resource.should(:password_hash) ]
		mysql_flush
	end
